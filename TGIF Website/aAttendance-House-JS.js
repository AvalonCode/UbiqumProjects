...

{
    ...
    <tr>
        <td>Total</td>
        <td>${dems.length + reps.length + inds.length}</td>	<td>${((parseFloat(partyPercent(demsPercent).toFixed(2))+parseFloat(partyPercent(repsPercent).toFixed(2))+parseFloat(partyPercent(indsPercent).toFixed(2)))/3).toFixed(2)}%</td>
    </tr>`;
}
...
for(i=0; i<pctOrder.length; i++){
    ...
    <tr>
		...
		<td>${pctOrder[i].missed_votes}</td>
		<td>${pctOrder[i].missed_votes_pct + "%"}</td>
	</tr>`;
}
...
for(i=0; i<pctReverse.length; i++){
    ...
    <tr>
		...
		<td>${pctReverse[i].missed_votes}</td>
		<td>${pctReverse[i].missed_votes_pct + "%"}</td>
	</tr>`;
}
